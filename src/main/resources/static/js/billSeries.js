document.addEventListener("DOMContentLoaded", function () {
    ...
    const prefixField = document.getElementById("prefix");                // Input for billSeries prefix
    const startingNumberField = document.getElementById("startNumber");// Input for starting number
    const previewNumberEl = document.getElementById("previewNumber");     // Element for next billSeries number preview
    const saveBtn = document.getElementById("saveBtn");                   // Save button

    console.log("✅ Bill Settings App Loaded!");
    ...
    async function fetchData() {
        try {
            // Fetch billSeries settings
            const settingsResponse = await fetch("/billSeries/settings");
            if (!settingsResponse.ok) throw new Error("Failed to fetch settings");
            const settingsData = await settingsResponse.json();

            // Fetch last billSeries record (expects JSON with property "lastBillNumber")
            const lastBillResponse = await fetch("/billSeries/last-record");
            if (!lastBillResponse.ok)
                throw new Error(`Failed to fetch last record: ${lastBillResponse.status}`);
            const lastBillData = await lastBillResponse.json();

            console.log("Fetched Settings Data:", settingsData);
            console.log("Fetched Last BillSeries Record:", lastBillData);
            ...
            // Update next billSeries number preview from last billSeries record
            if (previewNumberEl) {
                previewNumberEl.textContent = lastBillData.lastInvoiceNumber || "INV1001";
            }
	...
	function updatePreview() {
	    ...
	    fetch("/billSeries/latest-number")
	        .then(response => {
	            if (!response.ok) {
	        ...
	        .then(data => {
	            if (data.nextBillNumber) {
	                previewNumberEl.textContent = data.nextBillNumber;  // ✅ Corrected
	                console.log("Next Bill Number: " + data.nextBillNumber);
	            }
	        })
	        .catch(error => {
	            console.error("Error fetching latest bill number:", error);
	            showNotification("Failed to fetch latest bill number", "error");
	        });
	}
    ...
    function resetForm() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch("/billSeries/settings/reset", { method: "POST" })
                    .then(response => {
                        if (response.ok) {
